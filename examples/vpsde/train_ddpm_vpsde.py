...
python train_ddpm_vpsde.py ^
    ...
    model.diffusion_model.resnet_block_groups=4 ^
    model.diffusion_model.dim_mults=[1,2,4] ^
    model.train_ds.name="fashion_mnist" ^
    model.train_ds.split="train" ^
    trainer.max_epochs=5 ^
    trainer.strategy=null ^
    exp_manager.name="VPSDE-DDPM" ^
    exp_manager.exp_dir="Experiments" ^
    exp_manager.create_wandb_logger=True ^
    exp_manager.wandb_logger_kwargs.name="VPSDE-DDPM" ^
    exp_manager.wandb_logger_kwargs.project="VPSDE-DDPM" ^
...
def main(cfg):
    ...
    model = ScoreSDE(cfg=cfg.model, trainer=trainer)
